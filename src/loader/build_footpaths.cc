...
using component_it = component_vec::const_iterator;
...
component_vec find_components(timetable& tt, footgraph const& fgraph) {
  ...
  std::generate(
    ...
    [i = 0UL]() mutable { 
      return std::pair<component_idx_t, location_idx_t>{kNoComponent, location_idx_t{i++}};
    });

  std::stack<uint32_t> stack;  // invariant: stack is empty
  for (auto i = location_idx_t{0U}; i < fgraph.size(); ++i) {
    if (fgraph[i].empty()) {
      tt.locations_.components_[location_idx_t{i}] = tt.locations_.next_component_idx_;
