...
struct travel_time_label {
  ...
  friend bool operator>(travel_time_label const& a, travel_time_label const& b) {
    return a.d_ > b.d_;
  }
...
void dijkstra(timetable const& tt,
  ...
  for (auto const& start : q.destination_) {
    for_each_meta(
        tt, q.dest_match_mode_, start.target_, [&](location_idx_t const x) {
          ...
          m = std::min(static_cast<travel_time_dist_t>(start.duration().count()), m);
        });
  }
  ...
  for (auto const& [l, duration] : min) {
    ...
    for_each_meta(tt, q.start_match_mode_, l, [&](location_idx_t const meta) {
      ...
      dists[to_idx(meta)].travel_time_ = std::min(d, dists[to_idx(meta)].travel_time_);
  ...
  while (!pq.empty()) {
    ...
    for (auto const& e : lb_graph[l.l_]) {
      ...
      if (new_dist < dists[to_idx(e.target())].travel_time_ &&
        ...
        dists[to_idx(e.target())].travel_time_ = static_cast<travel_time_dist_t>(new_dist);
        pq.push(travel_time_label(e.target(), static_cast<travel_time_dist_t>(new_dist)));
      }
    }
  }
}
...
void dijkstra(timetable const& tt,
  ...
  constexpr auto const kUnreachable = std::numeric_limits<uint8_t>::max();
  ...
  for (auto l = location_idx_t{0}; l != tt.n_locations(); ++l) {
    ...
    lb[to_idx(l)].transports_ = 
      d == kUnreachable ? lower_bound::kTransportsUnreachable : d;
  }
}
