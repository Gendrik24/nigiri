...
std::optional<journey::leg> find_start_footpath(timetable const& tt,
  ...
  auto const fp_target_time = round_times[0][to_idx(leg_start_location)];
...
void reconstruct_journey(timetable const& tt,
  ...
  auto const best = [&](std::uint32_t const k, location_idx_t const l) {
    return std::min(best_times[to_idx(l)], round_times[k][to_idx(l)]);
  };
  ...
  auto const find_entry_in_prev_round =
    ...
    for (auto i = 1U; i != n_stops; ++i) {
      ...
      if (is_better_or_eq(round_times[k - 1][to_idx(l)], event_time)) {
        ...
        trace(
            ...
            "{})={} > event_time={}\n",
            location{tt, l}, k, k - 1, best_times[to_idx(l)],
            round_times[k - 1][to_idx(l)], best(k - 1, l), event_time);
      }
      ...
      if (k == 1 && q.start_match_mode_ == location_match_mode::kEquivalent) {
        for (auto const& eq : tt.locations_.equivalences_[l]) {
          if (is_better_or_eq(round_times[k - 1][to_idx(eq)],
                              event_time)) {
            return journey::leg{
                SearchDir,
                timetable::stop{stop_seq[stop_idx]}.location_idx(),
  ...
  auto const get_legs =
    ...
    auto const curr_time = round_times[k][to_idx(l)];
              ...
              for (auto const& fp : kFwd ? tt.locations_.footpaths_in_[eq]
                ...
                if (fp_intermodal_dest.has_value()) {
                  ...
                  trace(
                      ...
                      round_times[k][to_idx(fp.target_)], fp.duration_,
                      location{tt, eq}, round_times[k][to_idx(eq)],
  ...
  auto init_fp = find_start_footpath<SearchDir>(tt, q, j, round_times);
