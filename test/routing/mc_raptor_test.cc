...
TEST_CASE("mc-raptor") {
  ...
  auto mc_raptor_state = routing::mc_raptor_search_state{};
  ...
  auto fwdp_r = routing::mc_raptor<criteria::biCriteria>{
      tt, mc_raptor_state, q};
  fwdp_r.route();

  std::stringstream ss_profile;
  ss_profile << "\n";
  for (auto const& x : mc_raptor_state.results_.at(0)) {
