...
struct search {
  ...
  Algo init(algo_state_t& algo_state) {
...
#if defined(NIGIRI_TRACING)
      for (auto l = location_idx_t{0}; l != tt_.n_locations(); ++l) {
        auto const lb = 
          state_.transfers_lower_bound_[to_idx(tt_.locations_.components_[l])];
    ...
    if constexpr (Algo::kUseLowerBounds || Algo::kUseReachValues) {
      ...
      stats_.travel_time_lb_time_ = static_cast<std::uint64_t>(UTL_TIMING_MS(lb));
