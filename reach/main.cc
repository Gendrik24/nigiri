...
int main(int argc, char** argv) {
  ...
  if (argc != 5 && argc != 4) {
    auto c =argc == 0U ? "nigiri-server" : argv[0];
    fmt::print("usage: {} [TIMETABLE_PATH] [START_TIME] [END_TIME] [TIMETABLE_OUT] or\nusage: {} [TIMETABLE_FILE] [REACH_STORE_IDX] [DURATION_MIN]", c, c);
    return 1;
  }

  utl::activate_progress_tracker("import");
  auto num_threads = 12U;

  timetable tt;
  if (argc == 4) {
    cista::file f{argv[1], "r"};
    tt = *timetable::read(f.content());

    auto reach_store_idx = std::stoul(argv[2]);
    auto duration = std::stoi(argv[3]);

    extend_reach_store_by(reach_store_idx_t{reach_store_idx}, duration_t{duration}, tt, num_threads);
    tt.write(argv[1]);
  } else {
    ...
    add_reach_store_for({from_time, to_time}, tt, num_threads);
